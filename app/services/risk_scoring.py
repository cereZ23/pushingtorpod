...
class RiskScoringEngine:
    ...
    def _score_all_findings(self, asset: Asset) -> Dict:
        ...
        """
        findings = (
            self.db.query(Finding).filter_by(asset_id=asset.id).filter(Finding.status == FindingStatus.OPEN).all()
        )
            ...
            recommendations.append(
                {
                    "priority": "critical",
                    "message": (
                        f"{severity_counts['critical']} critical vulnerabilities require immediate remediation"
                    ),
                }
            )
                ...
                recommendations.append(
                    {
                        ...
                        "message": (f"High-risk {port_name} service exposed on {asset.identifier}:{service.port}"),
                    }
                )
    ...
    def _score_asset_age(self, asset: Asset) -> Dict:
        ...
        if is_new and asset.first_seen:
            ...
            recommendations.append(
                {
                    "priority": "high",
                    "message": (f"New asset discovered {days} days ago - requires additional monitoring"),
                }
            )
...
def recalculate_tenant_risk(
    ...
    assets = db.query(Asset).filter_by(tenant_id=tenant_id, is_active=True).all()
