...
SENSITIVE_PATHS: list[dict[str, Any]] = [
    ...
    {"path": "/aws-exports.js",      "severity": "critical", "pattern": r"aws_cognito_identity_pool_id|aws_user_pools_id|aws_appsync|endpoint|aws_project_region"},
    ...
    {"path": "/.well-known/apple-app-site-association", "severity": "low", "pattern": r'"applinks"|"appIDs"|"apps"|"details"'},
    {"path": "/.well-known/assetlinks.json", "severity": "low", "pattern": r'"target"|"namespace"|"package_name"|"sha256_cert_fingerprints"'},
    ...
    # GCP
    {"path": "/gcloud/application_default_credentials.json", "severity": "critical", "pattern": r'"client_id"|"client_secret"|"type"'},
    {"path": "/service-account.json",    "severity": "critical", "pattern": r'"type"\s*:\s*"service_account"|"private_key"'},
    {"path": "/service-account-key.json","severity": "critical", "pattern": r'"type"\s*:\s*"service_account"|"private_key"'},
    {"path": "/google-credentials.json", "severity": "critical", "pattern": r'"type"\s*:\s*"service_account"|"private_key"'},
    {"path": "/firebase-adminsdk.json",  "severity": "critical", "pattern": r'"type"\s*:\s*"service_account"|"private_key"'},
    {"path": "/.config/gcloud/credentials.db", "severity": "critical", "pattern": None},
    {"path": "/.config/gcloud/application_default_credentials.json", "severity": "critical", "pattern": r'"client_id"|"type"'},
    # Azure
    ...
    {"path": "/appsettings.Production.json",  "severity": "critical", "pattern": r'"ConnectionString"|"Password"|"Secret"'},
    ...
    {"path": "/.well-known/oauth-authorization-server", "severity": "low", "pattern": r'"issuer"|"authorization_endpoint"'},
    ...
    {"path": "/FederationMetadata/2007-06/FederationMetadata.xml", "severity": "medium", "pattern": r"<EntityDescriptor|<fed:"},
    ...
    {"path": "/swagger-resources/configuration/ui",  "severity": "low",  "pattern": r'"deepLinking"|"displayOperationId"'},
    ...
    {"path": "/environments/environment.ts",     "severity": "high", "pattern": r"(apiKey|apiUrl|clientId|clientSecret|baseUrl)"},
    {"path": "/environments/environment.prod.ts","severity": "high", "pattern": r"(apiKey|apiUrl|clientId|clientSecret|baseUrl)"},
...
async def _run_scan_async(
    ...
    for asset, base_urls in targets:
        for base_url in base_urls:
            ...
            host_tasks.append(
                _scan_host(base_url, paths_to_scan, host_semaphore, global_semaphore)
            )
...
def run_sensitive_path_scan(
    ...
    try:
        ...
        tenant_logger.info(
            f"Sensitive path scan: {len(targets)} assets, "
        ...
        tenant_logger.error(
            f"Sensitive path scan failed: {exc}", exc_info=True
        )
        stats["status"] = "failed"
        stats["error"] = str(exc)
        stats["errors"] += 1
