...
def send_new_asset_alerts(tenant_id: int):
    ...
    try:
        ...
        new_assets = (
            db.query(Asset)
            .filter(Asset.tenant_id == tenant_id, Asset.first_seen >= cutoff, Asset.is_active == True)
            .all()
        )
        ...
        if len(new_assets) > settings.alert_max_per_run:
            tenant_logger.warning(
                f"New asset count ({len(new_assets)}) exceeds cap ({settings.alert_max_per_run}), sending summary alert"
            )
            alert = Alert(
                tenant_id=tenant_id,
...
def evaluate_alert_policies(tenant_id: int, events: list):
    ...
    try:
        ...
        policies = db.query(AlertPolicy).filter(AlertPolicy.tenant_id == tenant_id, AlertPolicy.enabled == True).all()
...
def _send_webhook_message(url: str, alert: Alert):
    ...
    if settings.webhook_secret:
        ...
        signature = hmac.new(settings.webhook_secret.encode(), body_bytes, hashlib.sha256).hexdigest()
