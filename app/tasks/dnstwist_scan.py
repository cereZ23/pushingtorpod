...
def run_dnstwist(self, tenant_id: int, domain_list: Optional[List[str]] = None):
    ...
    try:
        ...
        tenant_logger.info(f"Starting dnstwist scan for {len(domain_list)} domain(s)")

        findings_created = 0

        for domain in domain_list:
            try:
                findings_created += _scan_single_domain(db, tenant_id, domain, tenant_logger)
            except Exception as exc:
                tenant_logger.warning(f"DNSTwist scan failed for {domain}: {exc}")
                continue

        db.commit()

        tenant_logger.info(f"DNSTwist scan complete: {findings_created} finding(s) across {len(domain_list)} domain(s)")
        return {
        ...
        db.rollback()
        tenant_logger.error(f"DNSTwist scan failed for tenant: {exc}", exc_info=True)
        raise
    finally:
        db.close()
...
def _scan_single_domain(
    ...
    try:
        ...
        tenant_logger.info("dnstwist library not installed -- using fallback permutation generator")
...
def _run_basic_permutations(
    ...
    tenant_logger.info(f"Generated {len(candidates)} permutation(s) for {domain}")
    ...
    tenant_logger.info(f"Resolved {len(permutations)} registered permutation(s) for {domain}")
...
def _process_permutation_results(
    ...
    if not root_asset:
        tenant_logger.warning(f"Root asset not found for domain {domain} -- skipping findings")
        return 0
    ...
    for perm in permutations:
        ...
        if not perm.get("dns_a") and not perm.get("dns_aaaa") and not perm.get("dns_mx"):
            continue
        ...
        severity = FindingSeverity.HIGH if perm.get("dns_mx") else FindingSeverity.MEDIUM
