...
from app.tasks.pipeline_phases.version_cve import _phase_9e_version_cve
...
def _send_scan_notification(scan_run_id: int, status: str, stats: dict, tenant_id: int) -> None:
    ...
    try:
        ...
        partial = stats.get("partial_phases") or []
        degraded = status == "completed" and (partial or stats.get("completeness") == "partial")

        if degraded:
            emoji = ":warning:"
            status_label = "completed (partial coverage)"
        else:
            emoji = ":white_check_mark:" if status == "completed" else ":x:"
            status_label = status

        text = (
            f"{emoji} *Scan #{scan_run_id}* {status_label} "
            f"(tenant {tenant_id}, {duration}s)\n"
            f"Findings: {findings} | "
            f"Phases: {stats.get('phases_completed', '?')}/{stats.get('phases_total', '?')}"
        )
        if partial:
            text += "\n:warning: Partial phases: " + ", ".join(p.get("name", p.get("phase", "?")) for p in partial)
...
PHASE_DEFS = {
    ...
    "9e": {"name": "Version-CVE Inference", "required": False},
...
EXECUTION_PLAN = [
    ...
    "6b",  # Web crawling only. Phase 6c (sensitive_paths) and phase 7
    # (visual recon / screenshots) removed:
    # - 6c: leaked 10GB, redundant with Nuclei http/exposures/
    ...
    # Misconfig + Origin + Nuclei + DNSTwist + DAST (9d T3+authorized) + Version-CVE (9e) in parallel
    ["8", "8c", "9", "9b", "9d", "9e"],
...
def _classify_phase_outcome(result) -> tuple:
    """Classify a finished phase's result as COMPLETED or PARTIAL.
    ...
    if result.get("coverage_complete") is False:
        passes = result.get("truncated_passes") or []
        reason = (
            f"incomplete coverage — truncated: {', '.join(map(str, passes))}"
            if passes
            else "incomplete coverage — some templates did not run"
        )
...
def _mark_phase_outcome(db, scan_run_id: int, phase_id: str, result, pipeline_stats: dict, tenant_logger=None):
    """Persist a finished phase as COMPLETED or PARTIAL and roll partial phases up
...
def _run_single_phase(
    ...
    try:
        ...
        _mark_phase_outcome(db, scan_run_id, phase_id, result, pipeline_stats, tenant_logger)
...
def run_scan_pipeline(self, scan_run_id: int):
    ...
    try:
        ...
        pipeline_stats = {
            ...
            "phases_failed": 0,
            "phases_skipped": 0,
            "partial_phases": [],  # [{phase, name, reason}] — phases that ran with reduced coverage
            "assets_discovered": 0,
            "findings_created": 0,
                    ...
                    try:
                        for future in as_completed(futures, timeout=group_timeout):
                            ...
                            if error:
                                ...
                                _mark_phase_outcome(db, scan_run_id, pid, result, pipeline_stats, tenant_logger)
                                if isinstance(result, dict):
                                    pipeline_stats["assets_discovered"] += result.get("assets_discovered", 0)
                                    pipeline_stats["findings_created"] += result.get("findings_created", 0)
                                    pipeline_stats["relationships_created"] += result.get("relationships_created", 0)
...
def _execute_phase(
    ...
    if phase_id == "0":
        ...
        return _phase_6b_web_crawling(tenant_id, project_id, scan_run_id, db, tenant_logger, scan_tier)
    elif phase_id == "6c":
        return _phase_6c_sensitive_paths(tenant_id, project_id, scan_run_id, db, tenant_logger, scan_tier)
    elif phase_id == "7":
        ...
        return _phase_8c_origin_discovery(tenant_id, project_id, scan_run_id, db, tenant_logger)
    elif phase_id == "9":
        ...
        return _phase_9e_version_cve(tenant_id, project_id, scan_run_id, db, tenant_logger, scan_tier)
    elif phase_id == "10":
