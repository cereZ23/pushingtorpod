...
export const SCAN_TIERS: ScanTier[] = [
  {
    tier: 1,
    name: 'Safe',
    description: 'Non-intrusive recon. Top 100 ports, CDN/WAF detection, safe Nuclei templates, 10 req/s.',
    ports: 'Top 100',
    rate: '10 req/s',
  },
  {
    tier: 2,
    name: 'Moderate',
    description: 'Standard scan. DNS permutation (alterx+puredns), cloud enum, service fingerprinting, top 1000 ports, medium Nuclei templates, 50 req/s.',
    ports: 'Top 1000',
    rate: '50 req/s',
  },
  {
    tier: 3,
    name: 'Aggressive',
    description: 'Full scan. All tier 2 tools + interactsh OOB callbacks, all ports, all Nuclei templates, 100 req/s. Use with caution.',
    ports: 'Full (65535)',
    rate: '100 req/s',
  },
]
...
export const useScanStore = defineStore('scans', () => {
  ...
  async function fetchScanRuns(projectId: number): Promise<void> {
    ...
    try {
      ...
      if (err instanceof Error && (err.name === 'CanceledError' || err.name === 'AbortError')) return
      const message = err instanceof Error ? err.message : 'Failed to fetch scan runs'
      error.value = message
    } finally {
  ...
  async function fetchScanRun(runId: number): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to fetch scan run'
      error.value = message
    }
  }

  async function triggerScan(projectId: number, scanTier: number = 1): Promise<ScanRun | null> {
    if (!tenantId.value) {
      error.value = 'No tenant selected'
      return null
    }
  ...
  async function fetchProgress(runId: number): Promise<void> {
    ...
    try {
      const response = await apiClient.get<{ scan_run: ScanRun; phases: PhaseProgress[] }>(
  ...
  async function deleteScanRun(runId: number): Promise<boolean> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to delete scan run'
      error.value = message
      return false
    }
  }
