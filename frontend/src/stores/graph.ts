...
export const useGraphStore = defineStore('graph', () => {
  ...
  async function fetchNodes(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to fetch graph nodes'
      error.value = message
    } finally {
  ...
  async function fetchEdges(): Promise<void> {
    ...
    try {
      ...
      if (err instanceof Error && (err.name === 'CanceledError' || err.name === 'AbortError')) return
      const message = err instanceof Error ? err.message : 'Failed to fetch graph edges'
      error.value = message
    } finally {
  ...
  async function expandNode(assetId: number): Promise<NeighborResponse | null> {
    ...
    try {
      ...
      const existingEdgeKeys = new Set(
        edges.value.map(
          (e) => `${e.source_id}-${e.target_id}-${e.rel_type}`
        )
      )
