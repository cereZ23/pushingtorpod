...
const isRunning = computed(() => {
  return (
    scanStore.currentScanRun?.status === "running" ||
    scanStore.currentScanRun?.status === "pending"
  );
});
...
const PHASE_LABELS: Record<string, { name: string; description: string }> = {
  "0": {
    name: "Seed Ingestion",
    description: "Loading root domains and IP ranges",
  },
  "1": {
  ...
  "1d": {
    name: "Cloud Buckets",
    description: "Scanning for exposed S3/GCS/Azure buckets",
  },
  "1e": {
    name: "Cloud Enum",
    description: "Enumerating cloud assets via cloudlist (Tier 2+)",
  },
  "2": {
    name: "DNS Bruteforce",
    description: "DNS permutation & resolution via alterx + puredns (Tier 2+)",
  },
  "3": {
    name: "DNS Resolution",
    description: "Resolving A/AAAA/CNAME/MX records",
  },
  "4": {
    name: "HTTP Probing",
  ...
  "5b": {
    ...
    description: "Detecting CDN, WAF and cloud providers via cdncheck",
  },
  "5c": {
    name: "Service Fingerprint",
    description:
      "Protocol-level service fingerprinting via fingerprintx (Tier 2+)",
  },
  "6": {
    name: "Fingerprinting",
    description: "Technology and service fingerprinting",
  },
  "6b": { name: "Web Crawling", description: "Crawling web pages with Katana" },
  "6c": {
    name: "Sensitive Paths",
    description: "Checking for exposed files and directories",
  },
  "7": {
    name: "Visual Recon",
    description: "Taking screenshots of live web pages",
  },
  "8": {
  ...
  "11": {
    name: "Risk Scoring",
    description: "Calculating risk scores per asset",
  },
  "12": {
    name: "Diff & Alerting",
    description: "Detecting changes and sending notifications",
  },
};
...
function formatPhaseLabel(phase: string): string {
  return (
    ...
    phase.replace(/_/g, " ").replace(/\b\w/g, (c: string) => c.toUpperCase())
  );
}
...
<template>
  <div class="space-y-6">
    ...
    <!-- Loading state -->
    <div
      v-if="!scanStore.currentScanRun && !scanStore.error"
      role="status"
    ...
    <template v-if="scanStore.currentScanRun">
      ...
      >
        ...
        >
          <p class="text-sm text-red-700 dark:text-red-300">
            ...
            {{ scanStore.currentScanRun.error_message }}
          </p>
        </div>
      </div>
      ...
      >
        <h3
          class="text-lg font-semibold text-gray-900 dark:text-dark-text-primary mb-6"
        >
          Phase Progress
        </h3>
        ...
        <div
          v-if="
            scanStore.isLoadingProgress && scanStore.phaseProgress.length === 0
          "
        ...
        <div
          v-else-if="scanStore.phaseProgress.length === 0"
          class="text-center py-8 text-gray-500 dark:text-dark-text-secondary"
        >
        ...
        <div v-else class="relative">
          <!-- Vertical line -->
          <div
            class="absolute left-5 top-0 bottom-0 w-0.5 bg-gray-200 dark:bg-dark-border"
          />
          ...
          <div class="space-y-6">
            ...
            >
              ...
              >
                <!-- Pending: clock -->
                <svg
                  v-if="getPhaseIcon(phase.status) === 'clock'"
                  class="w-5 h-5"
                ...
                <!-- Running: spinner -->
                <svg
                  v-else-if="getPhaseIcon(phase.status) === 'spinner'"
                ...
                <!-- Completed: check -->
                <svg
                  v-else-if="getPhaseIcon(phase.status) === 'check'"
                  class="w-5 h-5"
                  :class="getPhaseStatusClass(phase.status)"
                  fill="none"
                  stroke="currentColor"
                ...
                <!-- Failed: x -->
                <svg
                  v-else-if="getPhaseIcon(phase.status) === 'x'"
                  class="w-5 h-5"
                  :class="getPhaseStatusClass(phase.status)"
                  fill="none"
                  stroke="currentColor"
      ...
      <div
        v-if="
          scanStore.currentScanRun.stats &&
          Object.keys(scanStore.currentScanRun.stats).length > 0
        "
        class="bg-white dark:bg-dark-bg-secondary rounded-lg border border-gray-200 dark:border-dark-border p-6"
      >
        <h3
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          >
            <dt
              class="text-sm font-medium text-gray-500 dark:text-dark-text-secondary capitalize"
            >
              {{ entry.key }}
            </dt>
            <dd
              class="mt-1 text-2xl font-semibold text-gray-900 dark:text-dark-text-primary"
            >
              {{ entry.value }}
            </dd>
          </div>
