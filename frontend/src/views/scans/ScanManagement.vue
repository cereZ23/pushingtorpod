...
import type { Project, ScanRun, ProjectSeed, ScanRunStatus } from '@/stores/scans'
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      <h2 class="text-2xl font-bold text-gray-900 dark:text-dark-text-primary">Scan Management</h2>
      <button
    ...
    <!-- Error Banner -->
    <div v-if="scanStore.error" role="alert" class="bg-red-50 dark:bg-red-900/20 p-4 rounded-md flex items-center justify-between">
      <p class="text-red-800 dark:text-red-200">{{ scanStore.error }}</p>
      <button @click="scanStore.clearError" class="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 text-sm">
        Dismiss
      </button>
    </div>
    ...
    <div class="flex gap-6">
      ...
      <div class="w-[300px] shrink-0">
        <div class="bg-white dark:bg-dark-bg-secondary rounded-lg border border-gray-200 dark:border-dark-border overflow-hidden">
          ...
          <!-- Loading state -->
          <div v-if="scanStore.isLoadingProjects" role="status" class="p-4 text-center text-gray-500 dark:text-dark-text-secondary">
            Loading projects...
          </div>

          <!-- Empty state -->
          <div v-else-if="scanStore.projects.length === 0" class="p-6 text-center">
            <p class="text-gray-500 dark:text-dark-text-secondary text-sm">No projects yet</p>
            <button
              @click="openCreateDialog"
      ...
      <div class="flex-1 min-w-0">
        <div v-if="!scanStore.selectedProject" class="bg-white dark:bg-dark-bg-secondary rounded-lg border border-gray-200 dark:border-dark-border p-12 text-center">
          ...
          <p class="mt-4 text-gray-500 dark:text-dark-text-secondary">Select a project to view scan runs</p>
        </div>

        <div v-else class="space-y-4">
          <!-- Project Header -->
          <div class="bg-white dark:bg-dark-bg-secondary rounded-lg border border-gray-200 dark:border-dark-border p-4">
            <div class="flex justify-between items-start">
              <div>
                <h3 class="text-lg font-semibold text-gray-900 dark:text-dark-text-primary">
                  {{ scanStore.selectedProject.name }}
                </h3>
                <p v-if="scanStore.selectedProject.description" class="text-sm text-gray-500 dark:text-dark-text-secondary mt-1">
          ...
          <div class="bg-white dark:bg-dark-bg-secondary rounded-lg border border-gray-200 dark:border-dark-border overflow-hidden">
            <!-- Loading -->
            <div v-if="scanStore.isLoadingRuns" role="status" class="p-8 text-center text-gray-500 dark:text-dark-text-secondary">
              Loading scan runs...
            </div>

            <!-- Empty -->
            <div v-else-if="scanStore.scanRuns.length === 0" class="p-8 text-center">
              <p class="text-gray-500 dark:text-dark-text-secondary">No scan runs yet for this project</p>
              <button
                @click="openTierDialog"
                class="mt-2 text-primary-600 dark:text-primary-400 text-sm hover:text-primary-700 dark:hover:text-primary-300"
              >
            ...
            <div v-else class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200 dark:divide-dark-border">
                ...
                <tbody class="bg-white dark:bg-dark-bg-secondary divide-y divide-gray-200 dark:divide-dark-border">
                  ...
                  >
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-mono text-gray-900 dark:text-dark-text-primary">
                      #{{ run.id }}
                    </td>
      ...
      >
        ...
        <div class="relative bg-white dark:bg-dark-bg-secondary rounded-lg shadow-xl w-full max-w-lg mx-4 border border-gray-200 dark:border-dark-border">
          <div class="px-6 py-4 border-b border-gray-200 dark:border-dark-border">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-dark-text-primary">Create New Project</h3>
          </div>
    ...
    <Teleport to="body">
      ...
      >
        ...
        <div class="relative bg-white dark:bg-dark-bg-secondary rounded-lg shadow-xl w-full max-w-md mx-4 border border-gray-200 dark:border-dark-border">
          <div class="px-6 py-4 border-b border-gray-200 dark:border-dark-border">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-dark-text-primary">Delete Scan Run</h3>
          </div>
          <div class="p-6">
            <p class="text-sm text-gray-600 dark:text-dark-text-secondary">
              Are you sure you want to delete scan run
              <span class="font-mono font-semibold">#{{ scanToDelete?.id }}</span>?
              This will permanently remove all phase results and observations. This action cannot be undone.
            </p>
          </div>
    ...
    <Teleport to="body">
      ...
      >
        ...
        <!-- Dialog -->
        <div class="relative bg-white dark:bg-dark-bg-secondary rounded-lg shadow-xl w-full max-w-2xl mx-4 border border-gray-200 dark:border-dark-border">
          <div class="px-6 py-4 border-b border-gray-200 dark:border-dark-border flex justify-between items-center">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-dark-text-primary">Select Scan Type</h3>
            <button @click="closeTierDialog" class="text-gray-400 hover:text-gray-500">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
          ...
          <div class="p-6">
            <div class="grid grid-cols-3 gap-4">
              <button
                ...
                :class="{
                  'border-green-300 dark:border-green-700 hover:border-green-500 hover:bg-green-50 dark:hover:bg-green-900/10': tier.tier === 1,
                  'border-yellow-300 dark:border-yellow-700 hover:border-yellow-500 hover:bg-yellow-50 dark:hover:bg-yellow-900/10': tier.tier === 2,
                  'border-red-300 dark:border-red-700 hover:border-red-500 hover:bg-red-50 dark:hover:bg-red-900/10': tier.tier === 3,
                }"
              >
                <!-- Icon -->
                <div class="mb-3">
                  <!-- Shield for Safe -->
                  <svg v-if="tier.tier === 1" class="w-10 h-10" :class="getTierIconClass(tier.tier)" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                  </svg>
                  <!-- Bolt for Moderate -->
                  <svg v-else-if="tier.tier === 2" class="w-10 h-10" :class="getTierIconClass(tier.tier)" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                  </svg>
                  <!-- Fire for Aggressive -->
                  <svg v-else class="w-10 h-10" :class="getTierIconClass(tier.tier)" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Name -->
                <h4 class="text-base font-semibold text-gray-900 dark:text-dark-text-primary mb-2">
                  {{ tier.name }}
                </h4>

                <!-- Description -->
                <p class="text-xs text-gray-500 dark:text-dark-text-secondary text-center mb-3">
                  {{ tier.description }}
                </p>

                <!-- Stats -->
                <div class="flex gap-3 text-xs text-gray-600 dark:text-dark-text-secondary">
