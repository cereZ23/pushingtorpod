...
const gradeColorClass = computed<string>(() => {
  ...
  const grade = executiveReport.value.grade.toUpperCase()
...
const isComplianceType = computed(() =>
  selectedReportType.value === 'soc2' || selectedReportType.value === 'iso27001'
)
...
async function generateReport(): Promise<void> {
  ...
  try {
    if (selectedReportType.value === 'executive') {
      ...
      const params: Record<string, string | number> = { limit: technicalLimit.value }
...
async function downloadReport(format: 'pdf' | 'docx'): Promise<void> {
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : `Failed to download ${format.toUpperCase()}`
    error.value = message
  } finally {
...
function getStatusBadgeClass(status: string): string {
  const colors: Record<string, string> = {
    ...
    suppressed: 'bg-gray-100 text-gray-600 dark:bg-gray-700/30 dark:text-gray-400',
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      <h2 class="text-2xl font-bold text-gray-900 dark:text-dark-text-primary">Reports</h2>
    ...
    <div class="bg-white dark:bg-dark-bg-secondary p-6 rounded-lg border border-gray-200 dark:border-dark-border">
      <h3 class="text-lg font-semibold text-gray-900 dark:text-dark-text-primary mb-4">Generate Report</h3>
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="flex items-end">
          ...
          >
            {{ isLoading || isDownloading ? 'Generating...' : isComplianceType ? 'Download PDF' : 'Generate' }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-if="isLoading" role="status" class="flex items-center justify-center h-64">
      <div class="text-gray-600 dark:text-dark-text-secondary">Generating report...</div>
    </div>
    ...
    <template v-if="executiveReport && !isLoading">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <!-- Risk Score Card -->
        <div class="bg-white dark:bg-dark-bg-secondary p-6 rounded-lg border border-gray-200 dark:border-dark-border">
          <p class="text-sm text-gray-600 dark:text-dark-text-secondary">Overall Risk Score</p>
          <div class="flex items-center mt-2">
            <p class="text-4xl font-bold text-gray-900 dark:text-dark-text-primary">
              {{ executiveReport.risk_score }}
            </p>
            <span class="text-sm text-gray-500 dark:text-dark-text-tertiary ml-1">/ 100</span>
          </div>
          <div class="mt-3 w-full h-3 bg-gray-200 dark:bg-dark-bg-tertiary rounded-full overflow-hidden">
            <div
              class="h-full rounded-full transition-all"
              :class="riskScoreColorClass"
              :style="{ width: `${Math.min(executiveReport.risk_score, 100)}%` }"
            />
          </div>
        </div>
        ...
        <!-- Grade Card -->
        <div class="bg-white dark:bg-dark-bg-secondary p-6 rounded-lg border border-gray-200 dark:border-dark-border">
          <p class="text-sm text-gray-600 dark:text-dark-text-secondary">Security Grade</p>
        ...
        <!-- Summary Stats Card -->
        <div class="bg-white dark:bg-dark-bg-secondary p-6 rounded-lg border border-gray-200 dark:border-dark-border">
          <p class="text-sm text-gray-600 dark:text-dark-text-secondary">Summary</p>
          <div class="mt-2 space-y-2">
            <div class="flex justify-between">
              <span class="text-sm text-gray-500 dark:text-dark-text-tertiary">Total Assets</span>
              <span class="text-sm font-medium text-gray-900 dark:text-dark-text-primary">
                {{ executiveReport.total_assets.toLocaleString() }}
              </span>
            </div>
            <div class="flex justify-between">
              <span class="text-sm text-gray-500 dark:text-dark-text-tertiary">Total Findings</span>
              <span class="text-sm font-medium text-gray-900 dark:text-dark-text-primary">
                {{ executiveReport.total_findings.toLocaleString() }}
              </span>
            </div>
            <div class="flex justify-between">
              <span class="text-sm text-gray-500 dark:text-dark-text-tertiary">Generated</span>
              <span class="text-sm font-medium text-gray-900 dark:text-dark-text-primary">
      ...
      <div class="bg-white dark:bg-dark-bg-secondary p-6 rounded-lg border border-gray-200 dark:border-dark-border">
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div
            v-for="(count, severity) in executiveReport.findings_by_severity"
            :key="severity"
      ...
      <div class="bg-white dark:bg-dark-bg-secondary rounded-lg border border-gray-200 dark:border-dark-border overflow-hidden">
        ...
        <div v-if="executiveReport.top_issues.length === 0" class="p-8 text-center">
          <p class="text-gray-500 dark:text-dark-text-secondary">No issues found</p>
        </div>
          ...
          <table class="min-w-full divide-y divide-gray-200 dark:divide-dark-border">
            ...
            <tbody class="bg-white dark:bg-dark-bg-secondary divide-y divide-gray-200 dark:divide-dark-border">
              ...
              >
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-dark-text-primary">
                  {{ issue.affected_assets }}
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-dark-text-secondary font-mono">
                  {{ issue.template_id || '-' }}
                </td>
              </tr>
            </tbody>
      ...
      <div class="bg-white dark:bg-dark-bg-secondary rounded-lg border border-gray-200 dark:border-dark-border overflow-hidden">
        ...
        <div v-if="executiveReport.recommendations.length === 0" class="p-8 text-center">
          <p class="text-gray-500 dark:text-dark-text-secondary">No recommendations at this time</p>
        </div>
        <ul v-else class="divide-y divide-gray-200 dark:divide-dark-border">
          ...
          >
            <div class="flex-shrink-0 w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-900/20 flex items-center justify-center">
              <span class="text-sm font-bold text-indigo-600 dark:text-indigo-400">{{ rec.priority }}</span>
            </div>
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2">
                <p class="text-sm font-medium text-gray-900 dark:text-dark-text-primary">{{ rec.title }}</p>
                <span
                  class="rounded-full px-2 py-1 text-xs font-semibold"
                  :class="getSeverityBadgeClass(rec.severity)"
                >
                  {{ rec.severity }}
                </span>
              </div>
              <p class="mt-1 text-sm text-gray-500 dark:text-dark-text-secondary">{{ rec.description }}</p>
            </div>
          </li>
        </ul>
      </div>
    ...
    <template v-if="technicalReport && !isLoading">
      ...
      <div class="bg-white dark:bg-dark-bg-secondary p-4 rounded-lg border border-gray-200 dark:border-dark-border">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-6">
            <div>
              <span class="text-sm text-gray-500 dark:text-dark-text-tertiary">Total Findings</span>
              <span class="ml-2 text-lg font-bold text-gray-900 dark:text-dark-text-primary">
                {{ technicalReport.total }}
              </span>
            </div>
            <div v-if="technicalReport.severity_filter">
              <span class="text-sm text-gray-500 dark:text-dark-text-tertiary">Filtered by</span>
              <span
                class="ml-2 rounded-full px-2 py-1 text-xs font-semibold"
                :class="getSeverityBadgeClass(technicalReport.severity_filter as SeverityLevel)"
              >
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div class="flex items-end">
            <p class="text-sm text-gray-500 dark:text-dark-text-tertiary">
              Showing {{ filteredFindings.length }} of {{ technicalReport.findings.length }} findings
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-dark-bg-secondary rounded-lg border border-gray-200 dark:border-dark-border overflow-hidden">
        <div v-if="filteredFindings.length === 0" class="p-8 text-center">
          <p class="text-gray-500 dark:text-dark-text-secondary">No findings match current filters</p>
        </div>
        <div v-else class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200 dark:divide-dark-border">
            <thead class="bg-gray-50 dark:bg-dark-bg-tertiary">
            ...
            <tbody class="bg-white dark:bg-dark-bg-secondary divide-y divide-gray-200 dark:divide-dark-border">
              ...
              >
                <td class="px-6 py-4">
                  ...
                  <div class="text-xs text-gray-500 dark:text-dark-text-secondary mt-0.5">
                    <span v-if="finding.cve_id" class="font-mono">{{ finding.cve_id }}</span>
                    <span v-if="finding.cve_id && finding.template_id"> | </span>
                    <span v-if="finding.template_id" class="font-mono">{{ finding.template_id }}</span>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <span
                ...
                <td class="px-6 py-4">
                  ...
                  <div class="text-xs text-gray-500 dark:text-dark-text-secondary capitalize">
                    {{ finding.asset_type }}
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                ...
                <td class="px-6 py-4">
                  <div v-if="finding.evidence && Object.keys(finding.evidence).length > 0">
                    <details class="text-xs">
                      <summary class="cursor-pointer text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300">
                        View evidence
                      </summary>
                      <pre class="mt-2 p-2 bg-gray-50 dark:bg-dark-bg-tertiary rounded text-xs text-gray-700 dark:text-dark-text-secondary overflow-auto max-w-xs max-h-32">{{ JSON.stringify(finding.evidence, null, 2) }}</pre>
                    </details>
                  </div>
                  <span v-else class="text-sm text-gray-400 dark:text-dark-text-tertiary">-</span>
                </td>
              </tr>
            </tbody>
          </table>
