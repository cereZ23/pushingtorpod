...
const visibleTypes = ref<Set<AssetNodeType>>(new Set(['domain', 'subdomain', 'ip', 'service', 'url']))
...
let simulation: d3.Simulation<GraphNode, SimulationEdge> | null = null
...
// ------------------------------------------------------------------ computed
const isLoading = computed(() => graphStore.isLoadingNodes || graphStore.isLoadingEdges)
...
const colorMap = computed(() => isDarkMode.value ? NODE_COLORS_DARK : NODE_COLORS)
...
function nodeRadius(riskScore: number): number {
  ...
  return MIN_NODE_RADIUS + (clamped / 100) * (MAX_NODE_RADIUS - MIN_NODE_RADIUS)
}
...
function buildSimulation(): void {
  ...
  simulation = d3.forceSimulation<GraphNode, SimulationEdge>(simNodes)
    .force(
      'link',
      d3.forceLink<GraphNode, SimulationEdge>(simEdges)
        .id((d) => d.id)
    ...
    .force('collision', d3.forceCollide<GraphNode>().radius((d) => nodeRadius(d.risk_score) + 2))
...
function updateLabelVisibility(): void {
  ...
  labelGroup.selectAll<SVGTextElement, GraphNode>('.node-label')
    .attr('display', (d) => shouldShowLabel(d) ? null : 'none')

  // Edge labels: only at high zoom
  labelGroup.selectAll<SVGTextElement, SimulationEdge>('.edge-label')
...
function updateVisibility(): void {
  ...
  linkGroup.selectAll<SVGLineElement, SimulationEdge>('line')
    .attr('display', (d) => {
      const srcId = getSourceId(d)
      const tgtId = getTargetId(d)
      return hiddenNodeIds.has(srcId) || hiddenNodeIds.has(tgtId) ? 'none' : null
    })
...
function renderGraph(simNodes: GraphNode[], simEdges: SimulationEdge[]): void {
  ...
  // --- Edges ---
  const links = linkGroup.selectAll<SVGLineElement, SimulationEdge>('line')
  ...
  const edgeLabels = labelGroup.selectAll<SVGTextElement, SimulationEdge>('.edge-label')
    .data(simEdges, (d) => `${getSourceId(d)}-${getTargetId(d)}-${d.rel_type}`)
  ...
  nodeEnter.merge(nodeEls)
    .attr('fill', (d) => nodeColor(d.type))

  // --- Node text labels (zoom-dependent visibility) ---
  const nodeLabels = labelGroup.selectAll<SVGTextElement, GraphNode>('.node-label')
    .data(simNodes, (d) => d.id)
...
// ------------------------------------------------------------------ drag handlers
function dragStarted(event: d3.D3DragEvent<SVGCircleElement, GraphNode, GraphNode>): void {
  if (!event.active) simulation?.alphaTarget(0.3).restart()
  event.subject.fx = event.subject.x
  event.subject.fy = event.subject.y
}

function dragged(event: d3.D3DragEvent<SVGCircleElement, GraphNode, GraphNode>): void {
  event.subject.fx = event.x
  event.subject.fy = event.y
}

function dragEnded(event: d3.D3DragEvent<SVGCircleElement, GraphNode, GraphNode>): void {
...
function centerOnNode(target: GraphNode): void {
  ...
  nodeGroup?.selectAll<SVGCircleElement, GraphNode>('circle')
    ...
    .attr('stroke', (d) =>
      d.id === target.id
        ? '#facc15'
        : isDarkMode.value ? '#1e293b' : '#ffffff'
    )
...
<template>
  <div class="relative h-full w-full flex flex-col bg-gray-50 dark:bg-dark-bg-primary">
    ...
    >
      ...
      <div class="relative w-80">
        ...
        >
          ...
          >
            ...
            <span class="ml-auto text-xs text-gray-500 dark:text-dark-text-tertiary capitalize">
              {{ result.type }}
            </span>
          </button>
        </div>
      ...
      <div class="flex items-center gap-1.5">
        <button
          ...
          :class="visibleTypes.has(item.type)
            ? 'border-transparent text-white'
            : 'border-gray-300 dark:border-dark-border text-gray-400 dark:text-gray-500 bg-transparent'"
          :style="visibleTypes.has(item.type) ? { backgroundColor: nodeColor(item.type) } : {}"
      ...
      <div class="flex items-center gap-4 text-sm">
        <div class="flex items-center gap-1.5">
          <span class="text-gray-500 dark:text-dark-text-secondary">Nodes:</span>
          <span class="font-semibold text-gray-900 dark:text-dark-text-primary">
            {{ graphStore.totalNodes }}
          </span>
        </div>
        <div class="flex items-center gap-1.5">
          <span class="text-gray-500 dark:text-dark-text-secondary">Edges:</span>
          <span class="font-semibold text-gray-900 dark:text-dark-text-primary">
        ...
        <div class="hidden lg:flex items-center gap-1.5">
          ...
          <span class="font-semibold text-severity-critical truncate max-w-[180px]">
            {{ statsHighestRisk }}
          </span>
        </div>
        <button
          ...
          :class="{ 'bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300': showStatsPanel }"
    ...
    >
      ...
      >
        ...
        <span class="text-xs text-gray-600 dark:text-dark-text-secondary">Expanding node...</span>
      </div>
      ...
      >
        <div class="text-center">
          ...
          <p class="mt-4 text-gray-500 dark:text-dark-text-secondary">No assets discovered yet.</p>
      ...
      >
        <div class="flex items-center gap-2 px-4 py-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md">
          <svg aria-hidden="true" class="w-4 h-4 text-red-600 dark:text-red-400 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd" />
          </svg>
          <span class="text-sm text-red-800 dark:text-red-200">{{ graphStore.error }}</span>
          <button
    ...
    >
      <div class="flex items-center justify-between mb-3">
        <p class="text-xs font-semibold text-gray-700 dark:text-dark-text-secondary uppercase tracking-wider">
          Graph Statistics
        </p>
        <button
      ...
      <!-- Node counts by type -->
      <div class="mb-3">
        <p class="text-[11px] font-medium text-gray-500 dark:text-dark-text-tertiary mb-1.5">Nodes by Type</p>
        <div class="space-y-1">
          <div
          ...
          >
            <div class="flex items-center gap-1.5">
              <span
                ...
                :style="{ backgroundColor: nodeColor((nodeType as string) as AssetNodeType) }"
              ></span>
              <span class="text-gray-600 dark:text-dark-text-secondary capitalize">{{ nodeType }}</span>
            </div>
            <span class="font-semibold text-gray-900 dark:text-dark-text-primary">{{ count }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="mb-3 pt-2 border-t border-gray-100 dark:border-dark-border">
        <p class="text-[11px] font-medium text-gray-500 dark:text-dark-text-tertiary mb-1.5">Edges by Type</p>
        <div class="space-y-1">
          <div
            ...
            class="flex items-center justify-between text-xs"
          >
            <span class="text-gray-600 dark:text-dark-text-secondary">{{ edgeLabel(edgeType as string) }}</span>
            <span class="font-semibold text-gray-900 dark:text-dark-text-primary">{{ count }}</span>
          </div>
        </div>
      </div>
      ...
      >
        <p class="text-[11px] font-medium text-gray-500 dark:text-dark-text-tertiary mb-1.5">Most Connected</p>
          ...
          >
            <div class="flex items-center gap-1.5 truncate mr-2">
              <span
                ...
                :style="{ backgroundColor: nodeColor(mc.type as AssetNodeType) }"
              ></span>
              <span class="text-gray-700 dark:text-dark-text-secondary truncate">{{ mc.identifier }}</span>
            </div>
            <span class="font-semibold text-gray-900 dark:text-dark-text-primary flex-shrink-0">{{ mc.connection_count }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    >
      <p class="text-xs font-semibold text-gray-700 dark:text-dark-text-secondary uppercase tracking-wider mb-2">
        Node Types
      </p>
      ...
      <div class="mt-3 pt-2 border-t border-gray-100 dark:border-dark-border space-y-0.5">
            ...
            <p>
              ...
              <span
                ...
                :class="{
                  ...
                  'text-orange-400': tooltip.node.risk_score >= 60 && tooltip.node.risk_score < 80,
                  'text-yellow-400': tooltip.node.risk_score >= 40 && tooltip.node.risk_score < 60,
