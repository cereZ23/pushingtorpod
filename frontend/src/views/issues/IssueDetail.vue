...
const COMMENT_REQUIRED_STATUSES: IssueStatus[] = [
  "false_positive",
  "accepted_risk",
];
...
async function executeTransition(
  ...
  const success = await issueStore.updateIssueStatus(
    issueStore.currentIssue.id,
    newStatus,
    comment,
  );
  if (success) {
...
async function handleSubmitComment(): Promise<void> {
  ...
  const result = await issueStore.addComment(
    issueStore.currentIssue.id,
...
const timeline = computed(
  (): Array<{
    ...
    data: IssueActivity | IssueComment;
  }> => {
    if (!issueStore.currentIssue) return [];
...
async function handleCreateTicket(): Promise<void> {
  if (!issueStore.currentIssue || issueStore.currentIssue.findings.length === 0)
    return;
...
<template>
  <div class="space-y-6">
    ...
    <!-- Loading -->
    <div
      v-if="issueStore.isLoadingDetail && !issueStore.currentIssue"
      role="status"
    ...
    <template v-if="issueStore.currentIssue">
      ...
      >
        <div class="flex items-start justify-between">
          <div>
            <h3
              class="text-xl font-semibold text-gray-900 dark:text-dark-text-primary"
            >
        ...
        <div
          ...
          class="mt-4 pt-4 border-t border-gray-200 dark:border-dark-border"
        >
          <p
            class="text-sm font-medium text-gray-500 dark:text-dark-text-secondary mb-2"
          >
            Move to:
          </p>
        ...
        <div class="mt-4 pt-4 border-t border-gray-200 dark:border-dark-border">
          <div class="flex items-center gap-3">
            <template v-if="ticketInfo">
              ...
              <span
                class="px-2 py-0.5 text-xs font-semibold rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400"
              >
                {{ ticketInfo.status }}
              </span>
              <button
            ...
            <template v-else>
              <button
                ...
                :disabled="
                  isCreatingTicket || !issueStore.currentIssue?.findings?.length
                "
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        >
          <h4
            class="text-sm font-semibold text-gray-700 dark:text-dark-text-secondary uppercase tracking-wider mb-4"
          >
            Details
          </h4>
          <dl class="space-y-3">
            <div class="flex justify-between">
              <dt class="text-sm text-gray-500 dark:text-dark-text-secondary">
                Risk Score
              </dt>
              <dd
                class="text-sm font-medium text-gray-900 dark:text-dark-text-primary flex items-center"
              >
                {{ issueStore.currentIssue.risk_score }}
                <div
                  class="ml-2 w-16 h-2 bg-gray-200 dark:bg-dark-bg-tertiary rounded-full overflow-hidden"
                >
                  <div
                    ...
                    :class="{
                      ...
                      'bg-orange-500':
                        issueStore.currentIssue.risk_score >= 60 &&
                        issueStore.currentIssue.risk_score < 80,
                      'bg-yellow-500':
                        issueStore.currentIssue.risk_score >= 40 &&
                        issueStore.currentIssue.risk_score < 60,
                      'bg-blue-500': issueStore.currentIssue.risk_score < 40,
                    }"
                    :style="{
                      width: `${Math.min(issueStore.currentIssue.risk_score, 100)}%`,
                    }"
                  />
                </div>
              </dd>
            ...
            <div class="flex justify-between">
              ...
              >
                {{ issueStore.currentIssue.affected_assets_count }}
              </dd>
            </div>
            <div class="flex justify-between">
              <dt class="text-sm text-gray-500 dark:text-dark-text-secondary">
              ...
              <dd
                class="text-sm font-medium text-gray-900 dark:text-dark-text-primary"
              >
                {{ issueStore.currentIssue.finding_count }}
              </dd>
            </div>
            <div class="flex justify-between">
              <dt class="text-sm text-gray-500 dark:text-dark-text-secondary">
            ...
            <div class="flex justify-between">
              ...
              <dd
                ...
                :class="
                  isSlaOverdue(issueStore.currentIssue.sla_due_at)
        ...
        >
          ...
          <p
            v-else
            class="text-sm text-gray-400 dark:text-dark-text-tertiary italic"
          >
            No root cause analysis provided yet.
          </p>

          <div
            v-if="issueStore.currentIssue.description"
            class="mt-4 pt-4 border-t border-gray-200 dark:border-dark-border"
          >
            <h4
              class="text-sm font-semibold text-gray-700 dark:text-dark-text-secondary uppercase tracking-wider mb-2"
            >
              Description
            </h4>
            <p
      ...
      <!-- Threat Intelligence -->
      <div
        v-if="issueStore.currentIssue.findings.length > 0"
        class="bg-white dark:bg-dark-bg-secondary rounded-lg border border-gray-200 dark:border-dark-border p-6"
      >
        <h4
          class="text-sm font-semibold text-gray-700 dark:text-dark-text-secondary uppercase tracking-wider mb-4"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <!-- CVE -->
          <div v-if="issueStore.currentIssue.root_cause?.startsWith('cve:')">
            <p class="text-xs text-gray-500 dark:text-dark-text-tertiary">
              CVE
            </p>
            <a
              :href="`https://nvd.nist.gov/vuln/detail/${issueStore.currentIssue.root_cause.replace('cve:', '').toUpperCase()}`"
              target="_blank"
              rel="noopener"
          ...
          <div v-if="firstFindingEpss">
            ...
            <p
              ...
              :class="
                firstFindingEpss > 0.5
                  ? 'text-red-600'
                  : firstFindingEpss > 0.1
                    ? 'text-orange-500'
                    : 'text-green-600'
              "
            >
              {{ (firstFindingEpss * 100).toFixed(1) }}%
            </p>
          </div>
          <!-- Matched URL -->
          <div v-if="firstFindingUrl">
            <p class="text-xs text-gray-500 dark:text-dark-text-tertiary">
              Detected At
            </p>
            <a
              :href="firstFindingUrl"
              target="_blank"
              rel="noopener"
        ...
        <div class="mt-4 pt-4 border-t border-gray-200 dark:border-dark-border">
          <p
            class="text-xs font-semibold text-gray-700 dark:text-dark-text-secondary uppercase mb-2"
          >
            How to Verify
          </p>
          <div
          ...
          >
            ...
            >
              ...
              >
                ...
                <td
                  class="px-6 py-3 text-sm text-gray-500 dark:text-dark-text-secondary"
                >
                  {{ finding.asset_identifier || "-" }}
                </td>
                <td class="px-6 py-3 text-sm">
                  <a
                    v-if="finding.matched_at"
                    :href="finding.matched_at"
                    target="_blank"
                    rel="noopener"
                    class="text-primary-600 dark:text-primary-400 hover:underline break-all text-xs"
                  >
                    {{
                      finding.matched_at.length > 60
                        ? finding.matched_at.slice(0, 60) + "..."
                        : finding.matched_at
                    }}
                  </a>
      ...
      >
        ...
        <div class="p-6">
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                <template v-if="item.type === 'comment'">
                  ...
                  <div
                    class="mt-1 p-3 bg-gray-50 dark:bg-dark-bg-tertiary rounded-md text-sm text-gray-700 dark:text-dark-text-primary whitespace-pre-wrap"
                  >
          ...
          <div
            v-else
            class="text-center py-4 mb-6 text-sm text-gray-500 dark:text-dark-text-secondary"
          >
            No activity yet
          </div>
