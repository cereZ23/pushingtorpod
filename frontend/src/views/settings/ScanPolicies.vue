...
async function saveEditSchedule(profile: ScanProfile) {
  const cron = editPreset.value === '__custom__'
...
function getTierBadgeClass(tier: number): string {
  if (tier === 1) return 'bg-green-100 text-green-700 dark:bg-green-900/20 dark:text-green-400'
  if (tier === 2) return 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/20 dark:text-yellow-400'
...
async function fetchScopes(): Promise<void> {
  ...
  try {
    ...
    if (isNotFoundError(err)) {
      ...
      const message = err instanceof Error ? err.message : 'Failed to load scopes'
      error.value = message
    }
  } finally {
...
async function handleUpdateSchedule(profile: ScanProfile, cron: string | null): Promise<void> {
  if (!currentTenantId.value || !selectedProjectId.value) return
...
async function handleRemoveScope(scope: ScopeRule): Promise<void> {
  ...
  try {
    ...
    if (!isNotFoundError(err)) {
      const message = err instanceof Error ? err.message : 'Failed to remove scope rule'
      error.value = message
    }
  }
}
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      <div>
        <h2 class="text-2xl font-bold text-gray-900 dark:text-dark-text-primary">Scan Policies</h2>
        <p class="text-sm text-gray-500 dark:text-dark-text-tertiary mt-1">Configure scan profiles, scheduling, and scope rules</p>
      </div>
    </div>
    ...
    <div class="flex gap-6">
      ...
      <div class="w-[260px] shrink-0">
        <div class="bg-white dark:bg-dark-bg-secondary rounded-lg border border-gray-200 dark:border-dark-border overflow-hidden">
          <div class="px-4 py-3 border-b border-gray-200 dark:border-dark-border">
            <h3 class="text-sm font-semibold text-gray-700 dark:text-dark-text-secondary uppercase tracking-wider">Projects</h3>
          </div>

          <div v-if="scanStore.isLoadingProjects" class="p-4 text-center text-gray-500 dark:text-dark-text-secondary text-sm">
            Loading projects...
          </div>

          <div v-else-if="scanStore.projects.length === 0" class="p-6 text-center">
            <p class="text-gray-500 dark:text-dark-text-secondary text-sm">No projects found</p>
            <p class="text-xs text-gray-400 dark:text-dark-text-tertiary mt-1">Create a project in Scan Management first</p>
          </div>
          ...
          <ul v-else class="divide-y divide-gray-200 dark:divide-dark-border">
            ...
            >
              <div class="text-sm font-medium text-gray-900 dark:text-dark-text-primary">{{ project.name }}</div>
              <div v-if="project.description" class="text-xs text-gray-500 dark:text-dark-text-secondary mt-1 truncate">
                {{ project.description }}
              </div>
            </li>
          </ul>
      ...
      <div class="flex-1 min-w-0 space-y-6">
        <!-- No project selected -->
        <div v-if="!selectedProject" class="bg-white dark:bg-dark-bg-secondary rounded-lg border border-gray-200 dark:border-dark-border p-12 text-center">
          ...
          <p class="mt-4 text-gray-500 dark:text-dark-text-secondary">Select a project to configure scan policies</p>
        </div>
        ...
        <template v-else>
          ...
          <div class="bg-white dark:bg-dark-bg-secondary rounded-lg border border-gray-200 dark:border-dark-border overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-dark-border flex justify-between items-center">
              <h3 class="text-lg font-semibold text-gray-900 dark:text-dark-text-primary">Scan Profiles</h3>
              <button
            ...
            <div v-if="isLoadingProfiles" class="p-6 text-center text-gray-500 dark:text-dark-text-secondary">
              Loading profiles...
            </div>

            <div v-else-if="profiles.length === 0" class="p-6 text-center">
              <p class="text-gray-500 dark:text-dark-text-secondary text-sm">No scan profiles configured</p>
              <button
                @click="showCreateProfile = true"
              ...
              <table class="min-w-full divide-y divide-gray-200 dark:divide-dark-border">
                <thead class="bg-gray-50 dark:bg-dark-bg-tertiary">
                  <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-dark-text-secondary uppercase tracking-wider">Tier</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-dark-text-secondary uppercase tracking-wider">Ports</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-dark-text-secondary uppercase tracking-wider">Rate</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-dark-text-secondary uppercase tracking-wider">Schedule</th>
                ...
                <tbody class="bg-white dark:bg-dark-bg-secondary divide-y divide-gray-200 dark:divide-dark-border">
                  <tr v-for="profile in profiles" :key="profile.id" class="hover:bg-gray-50 dark:hover:bg-dark-bg-tertiary">
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="text-sm font-medium text-gray-900 dark:text-dark-text-primary">{{ profile.name }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <span
                        class="px-2 py-0.5 text-xs font-semibold rounded-full"
                        :class="getTierBadgeClass(profile.tier)"
                      >
                        Tier {{ profile.tier }} - {{ getTierLabel(profile.tier) }}
                      </span>
                    </td>
                    ...
                    <td class="px-6 py-4">
                      <template v-if="editingScheduleId === profile.id">
                        <div class="flex items-center gap-2">
                          ...
                          />
                          <button @click="saveEditSchedule(profile)" class="text-primary-600 dark:text-primary-400 hover:text-primary-700 text-sm">Save</button>
                          <button @click="cancelEditSchedule" class="text-gray-500 dark:text-dark-text-tertiary hover:text-gray-700 text-sm">Cancel</button>
                        </div>
                      </template>
                      <template v-else>
                        ...
                        <span
                          v-if="profile.schedule_cron && !schedulePresets.find(p => p.value === profile.schedule_cron)"
          ...
          <div class="bg-white dark:bg-dark-bg-secondary rounded-lg border border-gray-200 dark:border-dark-border overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-dark-border flex justify-between items-center">
              <h3 class="text-lg font-semibold text-gray-900 dark:text-dark-text-primary">Scope Rules</h3>
              <button
            ...
            <div v-if="isLoadingScopes" class="p-6 text-center text-gray-500 dark:text-dark-text-secondary">
              Loading scope rules...
            </div>

            <div v-else-if="scopes.length === 0" class="p-6 text-center">
              <p class="text-gray-500 dark:text-dark-text-secondary text-sm">No scope rules defined. All discovered assets will be in scope.</p>
            </div>

            <div v-else class="divide-y divide-gray-200 dark:divide-dark-border">
              <div
              ...
              >
                <div class="flex items-center gap-3">
                  <span
                    class="px-2 py-0.5 text-xs font-semibold rounded-full"
                    :class="scope.rule_type === 'include'
                      ? 'bg-green-100 text-green-700 dark:bg-green-900/20 dark:text-green-400'
                      : 'bg-red-100 text-red-700 dark:bg-red-900/20 dark:text-red-400'"
                  >
                    {{ scope.rule_type }}
                  </span>
                  <span class="text-sm font-mono text-gray-900 dark:text-dark-text-primary">{{ scope.pattern }}</span>
                  <span v-if="scope.description" class="text-xs text-gray-500 dark:text-dark-text-tertiary">
                    -- {{ scope.description }}
                  </span>
                </div>
                <button
          ...
          <div class="bg-white dark:bg-dark-bg-secondary rounded-lg border border-gray-200 dark:border-dark-border p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-dark-text-primary mb-4">Global Rate Limiting</h3>
            <div class="flex items-center gap-6">
              <div class="flex-1">
                ...
                />
                <div class="flex justify-between text-xs text-gray-500 dark:text-dark-text-tertiary mt-1">
                  <span>1 req/s</span>
                  <span>250 req/s</span>
                  <span>500 req/s</span>
                </div>
              ...
              <div class="text-center min-w-[80px]">
                <p class="text-3xl font-bold text-primary-600 dark:text-primary-400">{{ globalRateLimit }}</p>
                <p class="text-xs text-gray-500 dark:text-dark-text-tertiary">req/s</p>
              </div>
            </div>
          </div>
        </template>
    ...
    <Teleport to="body">
      ...
      >
        <div class="absolute inset-0 bg-black/50" @click="showCreateProfile = false" />
        <div class="relative bg-white dark:bg-dark-bg-secondary rounded-lg shadow-xl w-full max-w-lg mx-4 border border-gray-200 dark:border-dark-border">
          <div class="px-6 py-4 border-b border-gray-200 dark:border-dark-border flex justify-between items-center">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-dark-text-primary">Create Scan Profile</h3>
            <button @click="showCreateProfile = false" class="text-gray-400 hover:text-gray-500">
          ...
          <form @submit.prevent="handleCreateProfile" class="p-6 space-y-4">
            <!-- Name -->
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-dark-text-secondary mb-1">Profile Name</label>
              <input
            ...
            <!-- Tier -->
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-dark-text-secondary mb-2">Scan Tier</label>
              <div class="grid grid-cols-3 gap-3">
                <label
                  v-for="tier in [1, 2, 3]"
                  :key="tier"
                  class="relative flex flex-col items-center p-3 rounded-lg border-2 cursor-pointer transition-all"
                  :class="newProfile.tier === tier
                    ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20'
                    : 'border-gray-200 dark:border-dark-border hover:border-gray-300 dark:hover:border-gray-600'"
                >
                  <input
                    v-model="newProfile.tier"
                    :value="tier"
                    type="radio"
                    name="tier"
                    class="sr-only"
                  ...
                  <span class="text-xs text-gray-600 dark:text-dark-text-secondary">{{ getTierLabel(tier) }}</span>
                </label>
              </div>
            </div>
            ...
            <!-- Port Scan Mode -->
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-dark-text-secondary mb-1">Port Scan Mode</label>
              <select
                v-model="newProfile.port_scan_mode"
                class="w-full px-3 py-2 border border-gray-300 dark:border-dark-border rounded-md text-gray-900 dark:text-dark-text-primary dark:bg-dark-bg-tertiary focus:outline-none focus:ring-2 focus:ring-primary-500"
              >
                <option v-for="mode in portScanModes" :key="mode.value" :value="mode.value">{{ mode.label }}</option>
              </select>
            </div>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-dark-text-secondary mb-1">
                Max Rate (req/s): <span class="font-bold text-primary-600 dark:text-primary-400">{{ newProfile.max_rate }}</span>
              </label>
              <input
                v-model.number="newProfile.max_rate"
                type="range"
                min="1"
                max="300"
                step="1"
            ...
            <!-- Timeout -->
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-dark-text-secondary mb-1">Timeout (seconds)</label>
              <input
                v-model.number="newProfile.timeout_seconds"
                type="number"
                min="30"
                max="7200"
                class="w-full px-3 py-2 border border-gray-300 dark:border-dark-border rounded-md text-gray-900 dark:text-dark-text-primary dark:bg-dark-bg-tertiary focus:outline-none focus:ring-2 focus:ring-primary-500"
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-dark-text-secondary mb-2">Schedule</label>
              ...
              <div v-if="isCustomSchedule" class="mt-2">
                ...
                />
                <p class="text-xs text-gray-500 dark:text-dark-text-tertiary mt-1">Standard 5-field cron expression</p>
              </div>
            </div>
    ...
    <Teleport to="body">
      <div
        ...
        class="fixed inset-0 z-50 flex items-center justify-center"
      >
        <div class="absolute inset-0 bg-black/50" @click="showAddScope = false" />
        <div class="relative bg-white dark:bg-dark-bg-secondary rounded-lg shadow-xl w-full max-w-md mx-4 border border-gray-200 dark:border-dark-border">
          <div class="px-6 py-4 border-b border-gray-200 dark:border-dark-border flex justify-between items-center">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-dark-text-primary">Add Scope Rule</h3>
            <button @click="showAddScope = false" class="text-gray-400 hover:text-gray-500">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
          ...
          <form @submit.prevent="handleAddScope" class="p-6 space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-dark-text-secondary mb-1">Rule Type</label>
              <div class="flex gap-4">
                <label class="flex items-center gap-2 cursor-pointer">
                  <input v-model="newScope.rule_type" type="radio" value="include" class="text-primary-600 focus:ring-primary-500" />
                  <span class="text-sm text-gray-700 dark:text-dark-text-secondary">Include</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                  <input v-model="newScope.rule_type" type="radio" value="exclude" class="text-primary-600 focus:ring-primary-500" />
                  <span class="text-sm text-gray-700 dark:text-dark-text-secondary">Exclude</span>
                </label>
              </div>
            </div>
