...
async function handleSubmit() {
  ...
  try {
    ...
    error.value = axiosErr.response?.data?.detail || axiosErr.message || 'Failed to reset password'
  } finally {
  ...
  <div class="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-dark-bg-primary py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
      ...
      <div v-if="success" class="bg-green-50 dark:bg-green-900/20 p-6 rounded-lg text-center">
        ...
        <p class="mt-4 text-green-800 dark:text-green-200 font-medium">Password reset successfully</p>
      ...
      <form v-else class="mt-8 space-y-6" @submit.prevent="handleSubmit">
        <div class="space-y-4">
          <div>
            <label for="new-password" class="block text-sm font-medium text-gray-700 dark:text-dark-text-secondary mb-1">
              New Password
            </label>
            <input
              id="new-password"
