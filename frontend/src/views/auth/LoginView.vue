...
async function handleLogin() {
  ...
  try {
    ...
    error.value =
      axiosErr.response?.data?.detail || axiosErr.message || "Login failed";
  } finally {
...
async function handleMfaVerify() {
  ...
  try {
    ...
    error.value =
      axiosErr.response?.data?.detail || axiosErr.message || "Invalid MFA code";
  } finally {
    isLoading.value = false;
  }
}
...
<template>
  <div
    class="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-dark-bg-primary py-12 px-4 sm:px-6 lg:px-8"
  >
    <div class="max-w-md w-full space-y-8">
      <div>
        <h2
          class="mt-6 text-center text-3xl font-extrabold text-gray-900 dark:text-dark-text-primary"
        >
          EASM Platform
        </h2>
        <p
          class="mt-2 text-center text-sm text-gray-600 dark:text-dark-text-secondary"
        >
      ...
      <form v-else class="mt-8 space-y-6" @submit.prevent="handleLogin">
        ...
        <div class="relative">
          <div class="absolute inset-0 flex items-center">
            <div
              class="w-full border-t border-gray-300 dark:border-dark-border"
            ></div>
          </div>
          <div class="relative flex justify-center text-sm">
            <span
              class="px-2 bg-gray-50 dark:bg-dark-bg-primary text-gray-500 dark:text-dark-text-secondary"
            >
              Or
            </span>
          </div>
