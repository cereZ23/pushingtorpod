...
const geolocatedPct = computed(() => {
  ...
  return Math.round((summaryData.value.total_geolocated / summaryData.value.total_assets) * 100)
})
...
async function loadGeoData(): Promise<void> {
  ...
  try {
    ...
    // (it was hidden via v-show during loading)
    if (mapInstance) {
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      <div>
        <h2 class="text-2xl font-bold text-gray-900 dark:text-dark-text-primary">Geographic Map</h2>
    ...
    <div class="bg-white dark:bg-dark-bg-secondary p-4 rounded-lg border border-gray-200 dark:border-dark-border">
      <div class="flex flex-wrap items-center gap-4">
        ...
        <div class="ml-auto text-sm text-gray-500 dark:text-dark-text-tertiary">
          Showing {{ filteredFeatures.length }} of {{ features.length }} geolocated assets
        </div>

        <!-- Legend -->
        <div class="flex items-center gap-3 text-xs text-gray-600 dark:text-dark-text-secondary">
    ...
    <div v-if="isLoading" class="flex items-center justify-center h-64">
      <div class="flex flex-col items-center gap-3">
        ...
        <span class="text-gray-600 dark:text-dark-text-secondary">Loading geographic data...</span>
      </div>
    </div>
    ...
    >
      <div ref="mapContainerRef" class="w-full" style="height: 65vh; min-height: 400px;"></div>
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white dark:bg-dark-bg-secondary p-6 rounded-lg border border-gray-200 dark:border-dark-border">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm text-gray-600 dark:text-dark-text-secondary">Geolocated Assets</p>
              ...
              <div class="mt-2">
                <div class="w-full h-2 bg-gray-200 dark:bg-dark-bg-tertiary rounded-full overflow-hidden">
                  <div
        ...
        <div class="bg-white dark:bg-dark-bg-secondary p-6 rounded-lg border border-gray-200 dark:border-dark-border">
          <h3 class="text-sm font-semibold text-gray-900 dark:text-dark-text-primary mb-3">
            Top Countries
          </h3>
          <div v-if="topCountries.length > 0" class="space-y-2">
            ...
            >
              <div class="flex items-center gap-2">
                <span class="text-base leading-none">{{ countryCodeToFlag(country.country_code) }}</span>
                <span class="text-gray-700 dark:text-dark-text-secondary">{{ country.country_code }}</span>
              </div>
              <div class="flex items-center gap-3">
                <span class="font-semibold text-gray-900 dark:text-dark-text-primary">{{ country.count }}</span>
                <span
          ...
          <p v-else class="text-sm text-gray-500 dark:text-dark-text-tertiary">No country data available</p>
        </div>
        ...
        <div class="bg-white dark:bg-dark-bg-secondary p-6 rounded-lg border border-gray-200 dark:border-dark-border">
          ...
          <div class="space-y-3">
            ...
            <div v-if="summaryData.cloud_providers.length > 0">
              <p class="text-xs font-medium text-gray-500 dark:text-dark-text-tertiary uppercase tracking-wider mb-1">Cloud</p>
              <div
                v-for="provider in summaryData.cloud_providers.slice(0, 3)"
                :key="'cloud-' + provider.name"
                class="flex items-center gap-2 text-sm mb-1"
              >
                <span class="text-gray-700 dark:text-dark-text-secondary flex-1 truncate">{{ provider.name }}</span>
                <div class="w-20 h-1.5 bg-gray-200 dark:bg-dark-bg-tertiary rounded-full overflow-hidden">
                  <div
                    class="h-full bg-cyan-500 rounded-full"
                    :style="{ width: (provider.count / maxProviderCount) * 100 + '%' }"
                  ></div>
                </div>
                <span class="text-xs font-semibold text-gray-900 dark:text-dark-text-primary w-6 text-right">{{ provider.count }}</span>
              </div>
            </div>
            <!-- CDN Providers -->
            <div v-if="summaryData.cdn_providers.length > 0">
              <p class="text-xs font-medium text-gray-500 dark:text-dark-text-tertiary uppercase tracking-wider mb-1">CDN</p>
              <div
            ...
            <div v-if="summaryData.waf_providers.length > 0">
              <p class="text-xs font-medium text-gray-500 dark:text-dark-text-tertiary uppercase tracking-wider mb-1">WAF</p>
              <div
            ...
            <!-- Empty state -->
            <p
              v-if="summaryData.cloud_providers.length === 0 && summaryData.cdn_providers.length === 0 && summaryData.waf_providers.length === 0"
        ...
        <div class="bg-white dark:bg-dark-bg-secondary p-6 rounded-lg border border-gray-200 dark:border-dark-border">
          ...
          <div class="space-y-3">
            <!-- Critical -->
            <div class="flex items-center gap-3">
              <span class="w-16 text-sm text-gray-600 dark:text-dark-text-secondary text-right">Critical</span>
              <div class="flex-1 h-5 bg-gray-100 dark:bg-dark-bg-tertiary rounded-md overflow-hidden relative">
                <div
                  class="h-full rounded-md transition-all duration-700 ease-out bg-red-500"
                  :style="{ width: riskTotal > 0 ? (riskDistribution.critical / riskTotal) * 100 + '%' : '0%' }"
                ></div>
              </div>
              <span class="text-sm font-semibold text-gray-900 dark:text-dark-text-primary w-8 text-right">
                {{ riskDistribution.critical }}
              </span>
            </div>
            <!-- High -->
            <div class="flex items-center gap-3">
              <span class="w-16 text-sm text-gray-600 dark:text-dark-text-secondary text-right">High</span>
              <div class="flex-1 h-5 bg-gray-100 dark:bg-dark-bg-tertiary rounded-md overflow-hidden relative">
                <div
                  class="h-full rounded-md transition-all duration-700 ease-out bg-orange-500"
                  :style="{ width: riskTotal > 0 ? (riskDistribution.high / riskTotal) * 100 + '%' : '0%' }"
                ></div>
              </div>
              <span class="text-sm font-semibold text-gray-900 dark:text-dark-text-primary w-8 text-right">
            ...
            <!-- Medium -->
            <div class="flex items-center gap-3">
              <span class="w-16 text-sm text-gray-600 dark:text-dark-text-secondary text-right">Medium</span>
              <div class="flex-1 h-5 bg-gray-100 dark:bg-dark-bg-tertiary rounded-md overflow-hidden relative">
                <div
                  class="h-full rounded-md transition-all duration-700 ease-out bg-yellow-500"
                  :style="{ width: riskTotal > 0 ? (riskDistribution.medium / riskTotal) * 100 + '%' : '0%' }"
                ></div>
              </div>
              <span class="text-sm font-semibold text-gray-900 dark:text-dark-text-primary w-8 text-right">
            ...
            <!-- Low -->
            <div class="flex items-center gap-3">
              <span class="w-16 text-sm text-gray-600 dark:text-dark-text-secondary text-right">Low</span>
              <div class="flex-1 h-5 bg-gray-100 dark:bg-dark-bg-tertiary rounded-md overflow-hidden relative">
                <div
                  class="h-full rounded-md transition-all duration-700 ease-out bg-green-500"
                  :style="{ width: riskTotal > 0 ? (riskDistribution.low / riskTotal) * 100 + '%' : '0%' }"
                ></div>
              </div>
              <span class="text-sm font-semibold text-gray-900 dark:text-dark-text-primary w-8 text-right">
