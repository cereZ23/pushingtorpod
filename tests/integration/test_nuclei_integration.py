...
from app.models.database import Asset, AssetType, Finding, FindingSeverity, FindingStatus, Suppression, Tenant
...
class TestNucleiIntegration:
    ...
    def test_tenant(self, db_session: Session):
        """Create test tenant"""
        tenant = Tenant(name="Test Tenant", slug="test-tenant", contact_policy="Test policy")
    ...
    def test_asset(self, db_session: Session, test_tenant):
        """Create test asset"""
        asset = Asset(
            tenant_id=test_tenant.id, type=AssetType.DOMAIN, identifier="example.com", risk_score=0.0, is_active=True
        )
    ...
    def test_finding_repository_bulk_upsert(self, db_session: Session, test_tenant, test_asset):
        ...
        stored_findings = finding_repo.get_findings(tenant_id=test_tenant.id, asset_id=test_asset.id)
