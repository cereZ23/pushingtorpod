...
class TestFindingEndpoints:
    ...
    def test_list_findings_for_tenant(self, authenticated_client, test_tenant, tenant_with_findings):
        """Test listing all findings for a tenant"""
        response = authenticated_client.get(f"/api/v1/tenants/{test_tenant.id}/findings")
    ...
    def test_list_findings_with_severity_filter(self, authenticated_client, test_tenant, findings_mixed_severity):
        """Test filtering findings by severity level"""
        # Filter for critical findings only
        response = authenticated_client.get(
            f"/api/v1/tenants/{test_tenant.id}/findings", params={"severity": "critical"}
        )
        ...
        response = authenticated_client.get(f"/api/v1/tenants/{test_tenant.id}/findings", params={"severity": "high"})
    ...
    def test_list_findings_with_status_filter(self, authenticated_client, test_tenant, findings_various_statuses):
        """Test filtering findings by status"""
        # Filter for open findings
        response = authenticated_client.get(f"/api/v1/tenants/{test_tenant.id}/findings", params={"status": "open"})
    ...
    def test_get_finding_by_id_returns_details(self, authenticated_client, test_tenant, sample_finding):
        ...
        response = authenticated_client.get(f"/api/v1/tenants/{test_tenant.id}/findings/{sample_finding.id}")
    ...
    def test_findings_enforce_tenant_isolation(
        ...
        response = authenticated_client.get(f"/api/v1/tenants/{test_tenant.id}/findings/{other_finding_id}")

        # Should be 403 or 404
        assert response.status_code in [403, 404]

    def test_finding_deduplication_working(self, authenticated_client, test_tenant, duplicate_findings):
