...
def sample_tenant(test_db):
    ...
    tenant = Tenant(name="Test Tenant", slug="test-tenant", contact_policy="security@test.com", api_keys=None)
    test_db.add(tenant)
    test_db.commit()
...
class TestDiscoveryPipelineIntegration:
    ...
    def test_subfinder_integration(self, mock_store, mock_read, mock_execute):
        ...
        seed_data = {"domains": ["example.com"], "keywords": [], "asns": [], "ip_ranges": []}
    ...
    def test_full_pipeline_chain(self, mock_session_local, test_db, sample_tenant, sample_seeds):
        ...
        with (
            patch("app.utils.secure_executor.SecureToolExecutor.execute") as mock_execute,
            ...
            try:
                with patch("tempfile.NamedTemporaryFile") as mock_tempfile, patch("subprocess.run") as mock_run:
                    mock_file = MagicMock()
                    mock_file.name = output_file
                    mock_tempfile.return_value.__enter__.return_value = mock_file
                    mock_run.return_value = MagicMock(returncode=0)
...
class TestDatabaseOperationsIntegration:
    ...
    def test_event_repository_integration(self, test_db, sample_tenant):
        ...
        # Create asset
        asset = Asset(tenant_id=sample_tenant.id, identifier="test.example.com", type=AssetType.SUBDOMAIN)
        test_db.add(asset)
        test_db.commit()
...
class TestMultiTenantIsolation:
    ...
    def test_cross_tenant_access_prevented(self, mock_session_local, test_db):
        ...
        assets_data = [{"identifier": "secret.example.com", "type": AssetType.SUBDOMAIN, "raw_metadata": "{}"}]
...
class TestErrorRecoveryIntegration:
    ...
    def test_empty_results_handling(self, mock_session_local, test_db, sample_tenant):
        ...
        dnsx_result = {"resolved": [], "tenant_id": sample_tenant.id}
